...
public class PaintRed {
	private static final PaintRed REDPAINT_RECIPES = new PaintRed();
	private final Multimap<IBlockState, IBlockState> recipeList = ArrayListMultimap
			.create();

	public static PaintRed instance() {
		return REDPAINT_RECIPES;
	}
		...
		for (int x = 0; x < 16; x = x + 1) {
			if (x != 14) {
				...
				addPaint(Blocks.CARPET.getBlockState().getValidStates().get(x),
						Blocks.CARPET.getBlockState().getValidStates().get(14));
