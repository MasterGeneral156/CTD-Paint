...
import com.themastergeneral.ctdpaint.painting.PaintGreen;
...
public class GreenPaint extends PaintBrush {
	private Block containedBlock;

	public GreenPaint(String unlocalizedName, String modid) {
	...
	public EnumActionResult onItemUse(EntityPlayer playerIn, World worldIn,
		...
		if (!worldIn.isRemote) {
			...
			if (PaintGreen.instance().getRecipeResult(iblockstate) != Blocks.AIR
					.getDefaultState()) {
				stack.damageItem(1, playerIn);
				worldIn.setBlockState(pos,
						PaintGreen.instance().getRecipeResult(iblockstate));
